	...
	list_for_each_entry(s, &stubs, list) {
		if (s->addr == addr) {
			if (!strcmp(s->name, "handle_exception"))
				s->handler((struct pt_regs *)regs->a0, s->priv);
			else if (!strcmp(s->name, "do_ebreak"))
				s->handler((struct pt_regs *)regs->a0, s->priv);
			else if (s->handler)
				s->handler(regs, s->priv);
...
void default_stub_handler(struct pt_regs *regs, void *priv)
{
...
register_stub(const char *name,
	      void (*stub_handler)(struct pt_regs * regs, void *priv),
	      void *priv)
{
	struct stub *s;
	void *addr;
...
int __attribute__((section(".gos_stub_func")))
register_handle_exception_stub_handler(void (*handler)(struct pt_regs * regs, void *priv))
{
	return register_stub("handle_exception", handler, NULL);
}

int __attribute__((section(".gos_stub_func")))
register_ebreak_stub_handler(void (*handler)(struct pt_regs * regs, void *priv))
{
	return register_stub("do_ebreak", handler, NULL);
}
