...
static struct task *__switch_task(struct task_scheduler *ts)
{
	int cpu = sbi_get_cpu_id();
	struct task_ctrl *task_ctl = &per_cpu(tasks, cpu);
	struct task *task, *cur;
...
{
	__switch_task((struct task_scheduler *)data);
}
