...
{
	struct blk_device *bdev = node->bdev;
	struct ext4_priv *ext4 = (struct ext4_priv *)node->priv_data;
	struct ext4_super_block *sb = ext4->sb;
	int block_size = pow(2, 10 + sb->s_log_block_size);

	if (!bdev) {
		print("ext4: %s fail, bdev is NULL\n");
		return NULL;
	}

	return blk_get_buffer(bdev, block_size * block_num, len, actual_len);
}
...
static struct ext4_inode *get_ext4_inode(struct fs_node *node, unsigned int no)
{
	...
	inode =
		(struct ext4_inode *)blk_get_buffer(
					node->bdev,
					bg->inode_table_at * block_size + sb->s_inode_size * (no - 1),
					sizeof(struct ext4_inode), &ret_len
					);

	if (!inode || (ret_len != sizeof(struct ext4_inode)))
		return NULL;
...
static int ext4_fs_parse_meta(struct fs_node *node)
{
	struct blk_device *bdev = node->bdev;
	...
	if (!bdev) {
		print("ext4: parse meta fail, bdev is NULL\n");
		return -1;
	}
	...
	blk_read(bdev, ptr, (char *)sb, sizeof(struct ext4_super_block));
	...
	blk_read(bdev, ptr, (char *)gp_desc, sizeof(struct ext4_group_desc));
			 ...
			 struct fs_node *node, void *priv)
{
	...
	if (eh->eh_depth == 0) {
		...
		for (i = 0; i < eh->eh_entries; i++) {
			...
			remain_len = block_count * block_size;
			where = start_block;
			while (remain_len > 0) {
				int ret_len = 0;
				addr = get_block_addr(node, where, remain_len,
						      &ret_len);
				remain_len -= ret_len;
				where += ret_len / block_size;
				if (parse_file_entry((unsigned long)addr, ret_len, priv))
					return -1;
			}
		}
	...
	for (i = 0; i < eh->eh_entries; i++) {
		...
		h = (struct ext4_extent_header *)get_block_addr(node, leaf_block,
								sizeof(struct ext4_extent_header),
								&ret_len);
		if (!h || (ret_len != sizeof(struct ext4_extent_header)))
			return -1;
			...
			unsigned int count, struct dir_entry *parent)
{
	...
	while (n--) {
		...
		struct ext4_dir_entry_2 *entry;
		unsigned long where = start;

		addr = get_block_addr(node, where, remain, &ret_len);
		while (remain > 0) {
