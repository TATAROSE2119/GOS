...
static void mount_ext4_fs(struct blk_device *bdev)
{
...
int load_fs(u8 type, struct blk_device *bdev)
{
	...
	list_for_each_entry(node, &fs_root_nodes, list) {
		if (node->bdev == bdev)
			return node->id;
	}
