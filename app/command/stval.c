...
#define ACCESS_FAULT_ADDR 0xe0010000
...
static void ebreak_stub_handler(struct pt_regs *regs, void *priv)
{
	// regs->sbadaddr is the value of stval got from trap handler
	...
	if (regs->sepc == (unsigned long) internal_ebreak &&
		...
		(regs->sbadaddr == 0x0 || regs->sbadaddr == (unsigned long) internal_ebreak)) {
		print("TEST PASS\n");
	} else {
		print("TEST FAIL\n");
	}
...
static void load_misaligned_stub_handler(struct pt_regs *regs, void *priv)
{
	print("%s %d scause:0x%lx sepc:0x%lx stval:0x%lx\n",
		  __FUNCTION__, __LINE__, regs->scause, regs->sepc, regs->sbadaddr);
	...
	if (regs->scause == EXC_LOAD_MISALIGNED &&
		...
		(regs->sbadaddr == 0x0 || regs->sbadaddr == MISALIGNED_ADDR)) {
		print("TEST PASS\n");
	} else {
		print("TEST FAIL\n");
	}
...
static void store_misaligned_stub_handler(struct pt_regs *regs, void *priv)
{
	print("%s %d scause:0x%lx sepc:0x%lx stval:0x%lx\n",
		  __FUNCTION__, __LINE__, regs->scause, regs->sepc, regs->sbadaddr);
...
static void load_page_stub_handler(struct pt_regs *regs, void *priv)
{
	print("%s %d scause:0x%lx sepc:0x%lx stval:0x%lx\n",
		  __FUNCTION__, __LINE__, regs->scause, regs->sepc, regs->sbadaddr);
	//based on the objdump info of internal_readl, the sepc value should be internal_readl addr + 6
	if (regs->scause == EXC_LOAD_PAGE_FAULT &&
		regs->sepc == (unsigned long) internal_readl + 6 &&
		(regs->sbadaddr == INVALID_ADDRESS || regs->sbadaddr == 0x0 )) {
		print("TEST PASS\n");
	} else {
		print("TEST FAIL\n");
	}
...
static void store_page_stub_handler(struct pt_regs *regs, void *priv)
{
	print("%s %d scause:0x%lx sepc:0x%lx stval:0x%lx\n",
		  __FUNCTION__, __LINE__, regs->scause, regs->sepc, regs->sbadaddr);
	//based on the objdump info of internal_writel, the sepc value should be internal_writel addr + 20
	if (regs->scause == EXC_STORE_PAGE_FAULT &&
		regs->sepc == (unsigned long) internal_writel + 20 &&
		(regs->sbadaddr == 0x0 || regs->sbadaddr == INVALID_ADDRESS)) {
		print("TEST PASS\n");
	} else {
		print("TEST FAIL\n");
	}
...
static void load_access_stub_handler(struct pt_regs *regs, void *priv)
{
	print("%s %d scause:0x%lx sepc:0x%lx stval:0x%lx\n",
		  __FUNCTION__, __LINE__, regs->scause, regs->sepc, regs->sbadaddr);
	//based on the objdump info of internal_readl, the sepc value should be internal_readl addr + 6
	...
	if (regs->scause == EXC_LOAD_ACCESS &&
		regs->sepc == (unsigned long) internal_readl + 6 &&
		(regs->sbadaddr == 0x0 || regs->sbadaddr == ref_addr)) {
		print("TEST PASS\n");
	} else {
		print("TEST FAIL\n");
	}
...
{
	unsigned long val = 0;
	unsigned long start = 0xe0000000;
	unsigned long end = 0xe0100000;
	unsigned long attr = 0x8; /* pmpcfg = PMP_A_TOR, without R|W|X permission */

	SBI_CALL_3(SBI_PMP_INSERT_RANGE, start, end, attr);

	register_handle_exception_stub_handler(load_access_stub_handler);

	unsigned long addr = ACCESS_FAULT_ADDR;
	ref_addr = (unsigned long) ioremap((void*)addr, 4096, 0);
	val = internal_readl(ref_addr);
	print("0x%x = %x\n", ref_addr, val);
	return 0;
}

static void store_access_stub_handler(struct pt_regs *regs, void *priv)
{
	print("%s %d scause:0x%lx sepc:0x%lx stval:0x%lx\n",
		  __FUNCTION__, __LINE__, regs->scause, regs->sepc, regs->sbadaddr);
	//based on the objdump info of internal_writel, the sepc value should be internal_writel addr + 20
...
static void illegal_instruction_stub_handler(struct pt_regs *regs, void *priv)
{
	print("%s %d scause:0x%lx sepc:0x%lx stval:0x%lx\n",
		  __FUNCTION__, __LINE__, regs->scause, regs->sepc, regs->sbadaddr);
	print("sepc:0x%lx\n", (unsigned long) internal_illegal_instruction);
	if (regs->scause == EXC_INST_ILLEGAL &&
		regs->sepc == (unsigned long) internal_illegal_instruction &&
		(regs->sbadaddr == 0x0 || regs->sbadaddr == ILLEGAL_INSTRUCTION_ADDR)) {
		print("TEST PASS\n");
	} else {
		print("TEST FAIL\n");
	}
