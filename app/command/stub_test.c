...
static void handle_ebreak_stub_handler(struct pt_regs *regs, void *priv)
{
	print("%s %d scause:0x%lx sepc:0x%lx stval:0x%lx\n",
	      __FUNCTION__, __LINE__, regs->scause, regs->sepc, regs->sbadaddr);
}

static void handle_exception_stub_handler(struct pt_regs *regs, void *priv)
{
	print("%s %d scause:0x%lx sepc:0x%lx stval:0x%lx\n",
	      __FUNCTION__, __LINE__, regs->scause, regs->sepc, regs->sbadaddr);
}

static void handle_task_scheduler_event_stub_handler(struct pt_regs *regs, void *priv)
{
...
static void guest_implicit_load_event_handler(struct pt_regs *regs, void *priv)
{
...
static void guest_implicit_store_event_handler(struct pt_regs *regs, void *priv)
{
...
static int cmd_stub_handler(int argc, char *argv[], void *priv)
{
	...
	if (register_stub(argv[0], default_stub_handler, NULL)) {
...
static int cmd_stub_test_handler(int argc, char *argv[], void *priv)
{
	...
	if (!strncmp(argv[0], "handle_exception", sizeof("handle_exception"))) {
		...
		register_stub("task_scheduler_event_handler",
			      handle_task_scheduler_event_stub_handler, NULL);
