...
{
	int i, fd, hwirq, count;
	union imsic_test_ioctl imsic_test = { 0 };
	...
	imsic_test.set_count.count = TEST_COUNT;
	if (-1 == ioctl(fd, IMSIC_TEST_SET_COUNT, &imsic_test))
		return -1;

	count = imsic_test.set_count.ret_count;
	hwirq = imsic_test.set_count.hwirq_base;
	memset((char *)&imsic_test, 0, sizeof(union imsic_test_ioctl));

	for (i = 0; i < count; i++) {
		imsic_test.trigger.hwirq = hwirq + i;
		ioctl(fd, IMSIC_TEST_TRIGGER, &imsic_test);
		memset((char *)&imsic_test, 0, sizeof(union imsic_test_ioctl));
	}

	for (i = 0; i < count; i++) {
		imsic_test.affinity.hwirq = hwirq + i;
		imsic_test.affinity.cpu = 1;
		ioctl(fd, IMSIC_TEST_SET_AFFINITY, &imsic_test);
		memset((char *)&imsic_test, 0, sizeof(union imsic_test_ioctl));
	}
