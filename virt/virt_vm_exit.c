...
static int vcpu_do_trap_error(struct vcpu *vcpu, unsigned long saus)
{
	...
	if (vcpu_exist_remote_stub(vcpu)) {
		vcpu->exist_stub = 1;
		return 0;
	}
	...
	case EXC_STORE_GUEST_PAGE_FAULT:
		if (guest_ctx->hstatus & HSTATUS_SPV) {
			if(gstage_page_fault(vcpu, scause)){
				return vcpu_do_trap_error(vcpu, scause);
			}
		}else
			print
