...
#define N_PAGE_EXT(size, page_size)                                            \
	size % page_size == 0 ? size / page_size : (size / page_size + 1)
...
#define PAGE_WRITE_EXEC                                                        \
	__pgprot(_PAGE_BASE | _PAGE_READ | _PAGE_EXEC | _PAGE_WRITE)
...
enum napot_order { NAPOT_8K = 1, NAPOT_16K, NAPOT_32K, NAPOT_64K, NAPOT_MAX };
...
static inline unsigned long pte_mkcow(unsigned long pte)
{
	return pte | _PAGE_COW; // 还未定义
}

static inline int pte_is_cow(unsigned long pte)
{
	return !!(pte & _PAGE_COW); // 把非bool值改为bool值
}

static inline unsigned long pte_uncow_mkwrite(unsigned long pte)
{
...
static inline int pmd_none(unsigned long pmd) { return (pmd == 0); }
