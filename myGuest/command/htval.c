...
static void implicit_load_event_stub_handler(struct pt_regs *regs, struct virt_pt_regs *vregs)
{
	unsigned long gpa = GPA;

	print("%s %d scause:0x%lx sepc:0x%lx htval:0x%lx vsepc:0x%lx\n",
		  __FUNCTION__, __LINE__, regs->a1, regs->sepc, vregs->htval, vregs->vsepc);
	if (EXC_LOAD_GUEST_PAGE_FAULT == regs->a1 &&
		(vregs->htval == gpa >> 2 || vregs->htval == 0x0)) {
		print("TEST PASS\n");
	} else {
		print("TEST FAIL\n");
	}
...
static int implicit_load(void)
{
	...
	register_remote_stub("vcpu_do_trap_error",implicit_load_event_stub_handler);
...
static void implicit_store_event_stub_handler(struct pt_regs *regs, struct virt_pt_regs *vregs)
{
	unsigned long gpa = GPA;
...
static int implicit_store(void)
{
	...
	register_remote_stub("vcpu_do_trap_error",implicit_store_event_stub_handler);
