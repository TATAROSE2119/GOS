...
unsigned long get_phy_end(void) {
        ...
        return (mm_blocks.memory_block_size[last] + mm_blocks.memory_block_start[last]);
}
...
void memory_block_add(unsigned long base, unsigned long size) {
        ...
        while (i < size) {
                ...
                if (size - i > MAX_BYTE_PER_MAPS)
                        mm_blocks.memory_block_size[mm_blocks.avail] = MAX_BYTE_PER_MAPS;
                else
...
static void mm_reserved(unsigned long base, unsigned long size) {
        ...
        for (i = 0; i < n; i++) {
                ...
                if (!(base >= block_start && (base + size <= block_start + block_size))) {
                        if (size != 0)
...
void mm_init(struct device_init_entry *hw) {
        ...
        for (i = 0; i < n; i++) {
                ...
                print("Available Memory: phy_start_address:0x%lx, "
                      ...
                      mm_blocks.memory_block_start[i] + mm_blocks.memory_block_size[i],
                      mm_blocks.memory_block_size[i] / 1024,
                      nr_free_pages,
                      PAGE_SIZE);
        }
...
void *mm_alloc_fix(unsigned long addr, unsigned int size) {
        ...
        for (i = 0; i < n; i++) {
                ...
                if (!((unsigned long) addr >= start && (unsigned long) addr < start + len))
                        continue;
...
static void __free_pages_locked(unsigned long pa, int page_nr) {
        int n, total, i;
        struct mem_maps *mem_maps;
        unsigned long index, mem_map;
        int per_mem_map;

		n=mm_blocks.avail;
		for (i=0; i<n; i++) {
			unsigned long start=mm_blocks.memory_block_start[i];
			unsigned int len=mm_blocks.memory_block_size[i];
			...
			index=(pa-start)/PAGE_SIZE;
			if (index>=(unsigned long)total) {
				return;
			}
...
void reserved_mem_walk(void (*fn)(unsigned long addr, unsigned int nr, void *data), void *data) {
        ...
        for (i = 0; i < mm_blocks.reserved_cnt; i++) {
                fn(mm_blocks.memory_block_resv_start[i], mm_blocks.memory_block_resv_size[i], data);
        }
}
